...
const requiredIssue = { code: z.ZodIssueCode.custom, message: "Required when using custom email settings" };

export const MailSettingsSchema = z
  .object({
    email_use_global_settings: z.boolean(),
  ...
  .superRefine((data, ctx) => {
    if (data.email_use_global_settings) return;
    if (!data.email_default_sender) ctx.addIssue({ ...requiredIssue, path: ["email_default_sender"] });
    if (!data.email_smtp_host) ctx.addIssue({ ...requiredIssue, path: ["email_smtp_host"] });
    if (!data.email_smtp_port) ctx.addIssue({ ...requiredIssue, path: ["email_smtp_port"] });
  });

type MailSettings = z.infer<typeof MailSettingsSchema>;

const MailSettingsForm: React.FC<FormikProps<MailSettings>> = (formik) => {
  const { t } = useTranslation();
  return (
    <>
      <FormSwitch label={t("settings.email.useGlobalSettings")} name="email_use_global_settings" formik={formik} />
      {!formik.values.email_use_global_settings && (
        <>
...
export const TabMail: React.FC<{ nodeId: number; eventSettings: RestrictedEventSettings }> = ({ nodeId, eventSettings }) => {
  const { t } = useTranslation();
  ...
  return (
    <Formik initialValues={eventSettings as MailSettings} onSubmit={handleSubmit} validationSchema={toFormikValidationSchema(MailSettingsSchema)} enableReinitialize>
      {(formik) => (
        <Form onSubmit={formik.handleSubmit}>
          <Stack spacing={2}>
            <MailSettingsForm {...formik} />
            {formik.isSubmitting && <LinearProgress />}
            <Button type="submit" color="primary" variant="contained" disabled={formik.isSubmitting || Object.keys(formik.touched).length === 0}>{t("save")}</Button>
          </Stack>
        </Form>
      )}
    </Formik>
