...
WITH voucher_data AS (
    SELECT
        COALESCE(sum(CASE WHEN t.source_account = 6 THEN t.vouchers ELSE 0 END), 0) AS vouchers_issued,
        COALESCE(sum(CASE WHEN t.source_account != 6 THEN t.vouchers ELSE 0 END), 0) AS vouchers_spent
    FROM transaction t
    WHERE
        (from_timestamp IS NULL OR t.booked_at >= from_timestamp)
      AND (to_timestamp IS NULL OR t.booked_at <= to_timestamp)
)
SELECT
    COALESCE(vouchers_issued, 0) AS vouchers_issued,
    COALESCE(vouchers_spent, 0) AS vouchers_spent
FROM voucher_data;
$$ LANGUAGE SQL
STABLE
SECURITY INVOKER;
